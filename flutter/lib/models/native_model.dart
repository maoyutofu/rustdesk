...
class PlatformFFI {
  ...
  JzdeskImpl get ffiBind => _ffiBind;
  ...
  void _startListenEvent(JzdeskImpl jzdeskImpl) {
    final appType =
        _appType == kAppTypeDesktopRemote ? '$_appType,$kWindowId' : _appType;
    var sink = jzdeskImpl.startGlobalEventStream(appType: appType);
    sink.listen((message) {
      () async {
        try {
